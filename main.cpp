...
int nb_image = 10;
string folder_data = "/Users/baptisteguilleminot/Documents/FPT/Salt_creeping2/Data_dx/";
// Physical constants
...
int main(){
    if(visualisation){
        ...
        #pragma omp parallel
        for(int k = 1; k <= 15; k++){
            float dx_thread = dx * (0.5 + 0.1 * k); //Increase spacing for each parallel thread
            Case val_defaut = Case();
            Reseau res(nx,ny,dx_thread,val_defaut);
            for(int i = 0; i < 50; i++){
                for(int j = 0; j < ny; j++){
                    res[res.site_xy(i,j)].set_type(-1);
                }
            ...
            for(int i = 0; i < nb_image; i++){
                ...
                string nom_fichier = folder_data + "image" + to_string(i+1) + "_J" + to_string(dx_thread*1e7) + ".dat";
                res.enregistre_grille(nom_fichier);
                cout << i + 1 << " images enregistées.\n";
            }
            string nom_fichier_taille = folder_data + "taille_cristaux" + to_string(dx_thread*1e7) + ".dat";
            res.liste_taille_crist(nom_fichier_taille);
            cout << "Thread " << k << " finished with evaporation: " << dx_thread*1e7 << endl;
        }
    }
    return 0;
}